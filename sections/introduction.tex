...
The manual translation workflow typically involves several steps, illustrated in Figure \ref{fig:manual_workflow}. First, the translator reads and comprehends the manga content to understand context and narrative flow. Second, the text is translated from the source language to the target language while preserving meaning and cultural nuances. Third, the original text frames and speech bubbles are removed from the comic page using image editing software. Fourth, the translated text is pasted into the cleaned frames. Finally, the text is formatted and balanced within each bubble to ensure readability and visual harmony with the artwork.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/Data-Page-3.png}
